...
public class DynamicWebTable extends BaseClass {
	public static void main(String[] args) {
		...
		// loading URL
		driver.get("file:///D:/Desktop/handling%20webtable/webtable.html");
		// Printing table header of a web table assuming first row as header
		System.out
				.println("Printing all header of table assuming first row as header: ");
		List<WebElement> allHeadersOfTable = driver.findElements(By
				.xpath("//table[@name='BookTable']/tbody/tr[1]/th"));
		...
		System.out
				.println("=====================================================================");
		// Printing table header of a web table assuming no information about
		// header row
		System.out
				.println("Printing all header of table without information of row header ");
		List<WebElement> allHeadersOfTable1 = driver.findElements(By
				.xpath("//table[@name='BookTable']/tbody/tr/th"));
		System.out.println("Headers in table are below:");
		...
		System.out
				.println("=====================================================================");
		// Finding number of rows in a web table. We need to exclude header to
		// get actual number of data rows
		...
		System.out.println("Total data rows found in table:"
				+ (allRows.size() - 1));
		System.out
				.println("=====================================================================");
		...
		for (int i = 2; i <= allRows.size(); i++) {
			List<WebElement> allColumnsInRow = driver
					.findElements(By
							.xpath("//table[@name='BookTable']/tbody/tr[" + i
									+ "]/td"));
			System.out.println("Number of columns in " + (i - 1)
					+ " data row is:" + allColumnsInRow.size());
		}
		System.out
				.println("=====================================================================");
		...
		for (int i = 2; i <= allRows.size(); i++) {
			WebElement authorColumn = driver.findElement(By
					.xpath("//table[@name='BookTable']/tbody/tr[" + i
							+ "]/td[2]"));
			if (authorColumn.getText().toLowerCase().equalsIgnoreCase("Mukesh")) {
				WebElement bookNameColumns = driver.findElement(By
						.xpath("//table[@name='BookTable']/tbody/tr[" + i
								+ "]/td[1]"));
		...
		List<WebElement> allColumnsInRow = driver
				.findElements(By
						.xpath("//table[@name='BookTable']/tbody/tr/td[text()='Mukesh']/../td[1]"));
		...
		System.out
				.println("========================================================================");
		System.out
				.println("Books with price greater than and equal to 1000 are below:");
		for (int i = 2; i <= allRows.size(); i++) {
			WebElement priceColumn = driver.findElement(By
					.xpath("//table[@name='BookTable']/tbody/tr[" + i
							+ "]/td[4]"));
			if (Integer.parseInt(priceColumn.getText()) >= 1000) {
				WebElement bookName = driver.findElement(By
						.xpath("//table[@name='BookTable']/tbody/tr[" + i
								+ "]/td[1]"));
		...
		System.out
				.println("Directly printing column values of last row of table: ");
		List<WebElement> columnOfLastRow = driver.findElements(By
				.xpath("//table[@name='BookTable']/tbody/tr[last()]/td"));
		...
		List<WebElement> costColumns = driver.findElements(By
				.xpath("//table[@name='BookTable']/tbody/tr/td[4]"));
		int sum_price = 0;
		...
		WebElement colValue = driver.findElement(By
				.xpath("//table[@name='BookTable']/tbody/tr[2]/td[3]"));
		System.out.println("Cell Value : " + colValue.getText());
		System.out.println("========================================================================");
		System.out.println("Cell value using custom method: "
				+ DynamicWebTable.getColValue(2, 3, driver));
		// Printing column index based on column name
		List<WebElement> allHeadersOfTable2 = driver.findElements(By
				.xpath("//table[@name='BookTable']/tbody/tr[1]/th"));
		for (int k = 0; k > allHeadersOfTable2.size(); k++) {
			if (allHeadersOfTable2.get(k).getText().equalsIgnoreCase("price")) {
				System.out.println("Column index of Price column is: "
						+ (k + 1));
			}
		}
		driver.quit();
	...
	public static String getColValue(int row, int col, WebDriver driver) {
		WebElement colValue = driver.findElement(By
				.xpath("//table[@name='BookTable']/tbody/tr[" + row + "]/td["
						+ col + "]"));
